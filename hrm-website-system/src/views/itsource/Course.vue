<template>
	<section>
		...
		<el-table  @selection-change="handleSelectionChange" :data="courses" v-loading="listLoading"
			...
			<el-table-column prop="status" label="状态">
				<template scope="scope">
					<span v-if="scope.row.status==1" style="color: green">上线</span>
					<span v-if="scope.row.status==0" style="color: red">下线</span>
				</template>
			</el-table-column>
			...
			<el-table-column prop="startTime" label="开课时间" :formatter="formatStartTime">
			</el-table-column>
			<el-table-column prop="endTime" label="结课时间" :formatter="formatEndTime">
			</el-table-column>
		...
		<el-dialog title="新增" :visible.sync="addFormVisible"  :close-on-click-modal="false">
			<el-form :model="addForm" label-width="80px"  ref="addForm">
				...
				<el-form-item prop="logo" label="课程封面">
					...
					<el-upload
							class="upload-demo"
							action="http://localhost:1020/hrm/file/upload/fastdfs"
		...
		methods: {
            ...
            getCourseTypes(){
              this.$http.get("/course/courseType/treeData").then(result=>{
                  this.courseTypes = this.formatCourseTypes(result.data.resultObj);
              });
			},
			//格式化课程类型 使没有子级的不出现子级选框
			formatCourseTypes(courseTypes){
				courseTypes.forEach(courseType=>{
					let children = courseType.children;
					if (children.length==0){
						courseType.children = undefined;
					}else {
						this.formatCourseTypes(children);
					}
				})
				return courseTypes;
			},
            ...
            onLineCourse(){
				...
				this.$confirm('此操作将上架选中课程, 是否继续?', '提示', {
					confirmButtonText: '确定',
					cancelButtonText: '取消',
					type: 'warning'
				}).then(() => {
					let ids = this.selections.map(row=>{return row.id});
            ...
            offLineCourse(){
				...
				this.$confirm('此操作将下架选中课程, 是否继续?', '提示', {
					confirmButtonText: '确定',
					cancelButtonText: '取消',
					type: 'warning'
				}).then(() => {
